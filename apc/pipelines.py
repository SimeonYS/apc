...
class ApcPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('apc.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `apc`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM apc WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO apc (date, title, link, content)"
